        ...
        .forEach(relation =>
          relation.properties.relations.forEach(element => {
            ...
            element.properties.new =
              ['added', 'modifiedNew'].includes(
                relation.properties.changeType
              ) || false;
            relationMembers.push(element);
          })
        );
      console.log(featureCollection(relationMembers));
      this.map.addSource('relationMembers', {
        type: 'geojson',
        data: featureCollection(relationMembers)
      });
      this.map.addLayer({
        id: 'relation-menbers-line',
        type: 'line',
