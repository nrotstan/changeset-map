...
function displayDiff(id, featureMap) {
      ...
      elt(
        ...
        elt(
          'a',
          {
            ...
            href: '//www.openstreetmap.org/' + type + '/' + id
          },
          'OSM'
        )
      ),
  ...
  var metadataSubHeader = elt(
    'div',
    {},
    elt(
      'span',
      { class: 'cmap-inline-block cmap-subheader-label' },
      'Open with: '
    ),
    elt(
      'ul',
      ...
      elt(
        ...
        elt(
          ...
          {
            target: '_blank',
            class: 'cmap-hlist-item cmap-noselect cmap-pointer cmap-c-link-josm',
            href: getJOSMUrl(type, id)
          },
          'JOSM'
        )
      ),
      elt(
        ...
        elt(
          ...
          {
            target: '_blank',
            class: 'cmap-hlist-item cmap-noselect cmap-pointer cmap-c-link-id',
            href: getiDUrl(type, id)
          },
          'iD'
        )
      )
...
function getNodeTypeAbbreviation(nodeType) {
  return nodeType.charAt(0);
}

function getJOSMUrl(nodeType, osmId) {
  var typeMarker = getNodeTypeAbbreviation(nodeType);
  return (
    'http://127.0.0.1:8111/load_object?new_layer=true&objects=' +
    typeMarker +
    osmId
  );
}

function getiDUrl(nodeType, osmId) {
  var typeMarker = getNodeTypeAbbreviation(nodeType);
  return 'https://preview.ideditor.com/release/#id=' + typeMarker + osmId;
}

//Renders the markup for a table
function getDiffHTML(diff, ignoreList, header, subHeader) {
  ...
  if (header) {
    ...
    if (subHeader) {
      ...
      root.appendChild(elt('thead', {}, firstRow, secondRow));
    } else {
      root.appendChild(elt('thead', {}, firstRow));
    }
  }
        ...
        if (diff[prop].hasOwnProperty(type)) {
          ...
          if (prop === 'changeset' && type === 'modifiedOld') {
            var osmcha_link = elt(
              ...
              {
                target: '_blank',
                class: 'cmap-changeset-link',
                href: '//osmcha.mapbox.com/changesets/' + diff[prop][type]
              },
              diff[prop][type]
            );
            var td = elt('td', { class: propClass }, osmcha_link);
          } else {
